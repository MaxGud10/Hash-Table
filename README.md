...
4. **Округление результатов**
   ...
   * Итоговая запись: $$t = (\bar{t}_{округл} \pm \Delta t_{округл})$$
...
Приведем общую таблцу измерений на `ASM` & `Intrinsics`:

| Реализация       | Компилятор | Затрачено циклов       | Относительная погрешность |
|------------------|------------|-----------------------|--------------------------|
| **FastStrCmp**   | GCC        |$(8.17 \pm 0.02) \cdot 10^{10}$ |0.71%|
| (Intrinsics)     | CLANG      |$(7.96 \pm 0.06) \cdot 10^{10}$ |0.74%|
| **MyStrcmp**     | GCC        |$(8.55 \pm 0.03) \cdot 10^{10}$ |0.38%|
| (ASM)            | CLANG      |$(8.46 \pm 0.05) \cdot 10^{10}$ |0.53%|

- Ассемблерная реализация (```MyStrcmp```) дала прирост производительности в `1.95` раз
- Реализация на Intrinsics (```FastStrCmp```) показала ещё лучший результат — прирост в `~2.04` раз
...
Посмотрим как компилятор оптимизирует нашу программу через [GotBolt](https://godbolt.org/z/ee1neMWvK):

![GotBolt](img/gotbolt_new_intrin.png)

1.**Оптимизиция ветвлений**
```asm
; Intrinsics 
vpcmpeqb  ymm0, ymm0, YMMWORD PTR [rax]
vpmovmskb eax,  ymm0
cmp       eax, -1                        ; сразу проверяем полное совпадение
je .L6                                   ; выход при совпадении
...
jz .equal
```
 - В Intrinsics версии компилятор генерирует более эффективную проверку через cmp с константой -1 (0xFFFFFFFF), что на 1 инструкцию короче.
...
jz .equal
```
- `Intrinsics:` 3 AVX-инструкции + cmp.

  `ASM:`    2 AVX + vpmovmskb + xor + jz.
- Меньшее число инструкций сокращает время декодирования и выдачи в конвейер

3.**Более умная работа с памятью**

```asm
; Intrinsics:
lea rdx, [rcx+rax*8]             ; предвычисление адреса элемента 
vmovdqu  ymm0, YMMWORD PTR [rsi] ; загружаем данные строки
vpcmpeqb ymm0, ymm0, [rax]       ; сравнение с памятью напрямую

; ASM:
vmovdqu  ymm0,       [rdi]       ; загружаем первоую строку
vpcmpeqb ymm1, ymm0, [rsi]       ; обязательная загрузка второй в ymm1
```
- Предвычисление адресов: `lea` в Intrinsics снижает latency для доступа к элементам.
