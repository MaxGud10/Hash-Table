...
### Формулы для расчёта результатов измерений
...
4. **Округление результатов**
   *  Выбирают одну–две первые значащие цифры в значении случайной погрешности $\Delta t$
   * Округляют $\Delta t$ до этого количества значащих цифр:
   $$\Delta t -> \Delta t_{округл}$$

   * Округляют среднее значение $\bar{t}$ до этого же порядка:
   $$\bar{t} -> \bar{t}_{округл}$$

   * Итоговая запись: $$t = (\bar{t}_{округл} \pm \Delta {t}_{округл})$$
...
> [!WARNING]
> Для проверки на троттлинг CPU я запускал свою программу с максимальным приоритетом `nice -20` для минимизации влияния других процессов 

### GCC
![AIDA64_GCC](img/trol_gcc.png)
...
Можно заметить, что слова, использующиеся в речи и письме, не содержат более 32 символов. Так как каждый символ — это 8 бит, то возникает идея реализовать `strcmp()` при помощи ymm регистров. Два слова можно положить в два ymm регистра (так как каждое слово не более 256 бит), после чего сравнить их побайтово.

Чтобы корректно сравнивать строки, загрузим их в ymm регистры с помощью vmovdqu, затем используем vpcmpeqb для побайтового сравнения. Результат маскируется в битовую маску с помощью vpmovmskb, и если все байты совпадают (маска 0xFFFFFFFF), строки равны. В противном случае возвращается различие.

Этот подход позволяет эффективно сравнивать строки длиной до 32 байт, избегая поштучного сравнения в цикле и минимизируя количество операций.
...
MyStrcmp:
    ...
    vpmovmskb   eax,  ymm1          ; преобразуем результат в битовую маску

    xor         eax, 0xFFFFFFFF     ; инвертируем биты для проверки
    jz          .equal  
              
    mov         eax, 1              ; возвращаем 1 if разные
    ret

.equal:
    xor         eax, eax            ; возвращаем 0 if равны
    ret
```
...
Прирост производительности после замены стандартного `strcmp()` на `MyStrcmp()`колоссальный: он составляет x $2.06$
...
| Компилятор | Затрачено циклов      | Относительная погрешность |
|------------|-----------------------|--------------------------|
| GCC        | $(7.94 \pm 0.03) \cdot 10^{10}$ | 0.71%             |
| CLANG      | $(7.93 \pm 0.01) \cdot 10^{10}$  | 0.74%            |
...
- Ассемблерная реализация (```MyStrcmp```) дала прирост производительности в `2.06` раз
- Реализация на Intrinsics (```FastStrCmp```) показала ещё лучший результат — прирост в `~2.12` раз

Таким образом, интринсики оказались быстрее ассемблерной версии примерно на 6-7% относительно версии без оптимизации.

### Почему Intrinsics быстрее?

Посмотрим как компилятор оптимизирует нашу программу через [GotBolt](https://godbolt.org/z/zM5Ex19hz):

![GotBolt](img/gotbolt_dedloh.png)

1.**Оптимизация ветвления**

```asm
; Intrinsics 
not     eax
and     eax, 1
je      .L1              ; ранний выход при совпадении

; ASM
xor     eax, -1
and     eax, 1
ret                      ; всегда возвращает управление
```                                                    

- Изолированная функция не может оптимизировать ветвление
- На интрисиках компилятор лучше предсказывает ветления 

2.**Минимальное число регистров и инструкций
...
- Intrinsics повторно используют `ymm0` для загрузки и сравнения, вместо отдельного `ymm1`.

- Меньше uop’ов на декод и dispatch: 5 против 6 в ASM-версии.

### Встроенный в C ассемблер

Вторая функция, которую можно оптимизировать - это `strlen()` в `Murmurhash()`. На самом деле, нам очень сильно повезло, что в нашем датасете нет слов больше, чем 16 символов (максимальная длина слова - всего 14 букв). Это означает, что можно попробовать реализовать `strlen()`, который будет использовать векторные инструкции.
...
Из только что приведенного рисунка видно, что `MurmurHash()` занимает довольно много процессорного времени. Понятно, что необходимо как-то уменьшить это время, и только оптимизацией `strlen()` тут не обойтись. Специально для такого случая существует хэш-функция crc32, которая реализуется на аппаратном уровне. Если в нашем случае заменить `MurmurHash()` на аппаратно реализуемый crc32, то скорее всего, будет серьезный прирост. Проверим, так ли это.
...
***НО*** сначало сделаем промежуточные вычисления. Попробуем заменить `MurmurHash()` на `Crc32Hash()`, эта функция по первой части работы тоже хорошо себя проявила
